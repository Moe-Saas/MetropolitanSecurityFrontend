...
function Announce() {
  ...
  {
    await Axios.get('announce').then((response)=>{
        setInfo(response.data.announce)
    }).catch((error)=>{})
  }
  useEffect(()=>{
    getAnnounce()
  ...
  async function update(e)
  {
    if(FormValidation(e))
    {
        await Axios.post('announce/update',info,{withCredentials:true}).then((response)=>{
    ...
    <div>
      <div className=" flex justify-center">
        <div className="border-[1px] border-black w-full m-3 p-3 text-center rounded-md">
          <h1 className="font-bold text-xl md:text-3xl">Special Announcement</h1>
        </div>
      </div>
      {info?
      <form noValidate className="sm:w-full" onSubmit={update}>
        <div className="m-1">
          ...
          <br />
        <textarea name="announce" id="announce" className="bg-gray-100 w-full h-32 outline-none border-gray-300 border-[1px] rounded-md p-5 focus:bg-gray-50 focus:border-blue-400 text-[15px] my-2" onChange={handleInput} value={info.announce}></textarea>
